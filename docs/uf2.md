# Tema 2 - Gestors d’arxius web

1. [Introducció als contenidors lxc/lxd](intro-contenidors-lxc-lxd.md)
2. [Instal·lació de LAMP stack a Ubuntu 24.04](lamp-stack-ubuntu-24.04.md)
3. [Configuració de VirtualHost amb apache2](configuracio-virtual-host-apache2.md)
4. [Introducció a vagrant](vagrant-intro.md)
5. [Instal·lació i configuració de clouds](installacio-clouds.md)

## Pràctica
[Enunciat de la pràctica](pt-uf2.md)
